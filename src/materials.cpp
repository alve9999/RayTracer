...
ray metal::reflect(ray& incoming_ray) {
	glm::vec3 origin = incoming_ray.at(incoming_ray.reg_hit.t);
	glm::vec3 direction = (incoming_ray.direction - incoming_ray.reg_hit.normal * 2 * glm::dot(incoming_ray.direction, incoming_ray.reg_hit.normal)+(random_sphere_point()) * (1-fuzzines));
	ray outgoing_ray(direction, origin);
	outgoing_ray.reg_hit = incoming_ray.reg_hit;
	return outgoing_ray;
}
...
ray glass::reflect(ray& incoming_ray) {
	...
	if (k * sin > 1.0f ||schlink_aproximation(cos)>rand) {
		glm::vec3 origin = incoming_ray.at(incoming_ray.reg_hit.t);
		glm::vec3 direction = (incoming_ray.direction - incoming_ray.reg_hit.normal * 2 * glm::dot(incoming_ray.direction, incoming_ray.reg_hit.normal));
		ray outgoing_ray(direction, origin);
		outgoing_ray.reg_hit = incoming_ray.reg_hit;
		return outgoing_ray;
	}
	glm::vec3 outgoing_ray_a = (incoming_ray.direction+(incoming_ray.reg_hit.normal*cos))*k;
	glm::vec3 outgoing_ray_b = incoming_ray.reg_hit.normal * (-sqrt(1 - (glm::length(outgoing_ray_a) * glm::length(outgoing_ray_a))));
